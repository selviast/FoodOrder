...
class Admin extends CI_Controller 
{
    ...
    public function cariInvoice()
    {
        ...
        $data['title'] = 'Validasi'; 
        $data ['totalPesanan'] = $this->db->query(" select distinct id_pesanan, no_transaksi, nama, tgl_pesanan, nama_meja, lantai,sum(harga) 'total_harga' from transaksi join pesanan join meja on pesanan_id=id_pesanan and transaksi.id_meja=meja.id_meja where no_transaksi='$where';
        ")->result_array();                    
        $hasil = $this->db->query("select * from transaksi where no_transaksi='$where'")->row_array();
        if ($hasil <0) {
            echo 'nomor invoice tidak diketahui';
            //kei alert tambah mantul cell
